...
@WebServlet(name = "UpdateProductReview", value = "/product/review/update")
public class UpdateReviewServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (loggedUser != null) {
            request.getRequestDispatcher("/products/updateReview.jsp").forward(request, response);
        } else {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        Review review = ReviewServices.getReviewByUsername(loggedUser.getUsername());
        review.setComment(comment);

        boolean updatedStatus = ReviewServices.updateReview(loggedUser.getUsername(), product.getProductName(), review);
        if (updatedStatus) {
            response.sendRedirect(request.getContextPath() + "/products?updateReview=true");
        }
        else {
            response.sendRedirect(request.getContextPath() + "/products?error=ReviewCouldNotBeUpdated");
        }
    }
}
