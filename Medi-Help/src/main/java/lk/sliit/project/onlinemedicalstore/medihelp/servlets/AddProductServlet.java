...
@WebServlet(name = "AddProduct", value = "/products/add")
public class AddProductServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if ( (loggedUser != null) && (loggedUser.getRole().equals("Admin")) ) {
            request.getRequestDispatcher("/products/addProducts.jsp").forward(request, response);
        } else {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        boolean updatedStatus = ProductServices.addProduct(product);
        if (updatedStatus) {
            response.sendRedirect(request.getContextPath() + "/products?addProduct=true");
        }
        else {
            response.sendRedirect(request.getContextPath() + "/products?error=ProductAlreadyExists");
        }
    }
}
