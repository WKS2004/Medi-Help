...
import lk.sliit.project.onlinemedicalstore.medihelp.models.Pharmacist;
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if ("register".equalsIgnoreCase(action)) {
            ...
            if ("customer".equals(role)) {
                success = UserServices.registerUser(new Customer(username, firstName, lastName, displayName, (String) session.getAttribute("password"), (String) session.getAttribute("email")));
            } else {
                success = UserServices.registerUser(new Pharmacist(username, firstName, lastName, displayName, (String) session.getAttribute("password"), (String) session.getAttribute("email")));
            }
            response.sendRedirect(success ? request.getContextPath() + "/login?success=true" : request.getContextPath() + "/register?error=UserExists");
        }
    }
}
